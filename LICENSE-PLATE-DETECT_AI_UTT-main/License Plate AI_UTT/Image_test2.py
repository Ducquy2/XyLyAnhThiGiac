...
# Các hằng số này dùng để kiểm soát các tham số của thuật toán xử lý ảnh.
...
npaClassifications = npaClassifications.reshape(
    (npaClassifications.size, 1))  # Các dữ liệu phân loại và hình ảnh đã phẳng hóa được tải từ các tệp tin.
kNearest = cv2.ml.KNearest_create()  # instantiate KNN object
kNearest.train(npaFlattenedImages, cv2.ml.ROW_SAMPLE, npaClassifications)
######################### Mô hình KNN được tạo và huấn luyện với dữ liệu này.
...
###### Tìm kiếm và lọc các đường viền   #############
...
# Khoanh vùng và căn chỉnh biển số
